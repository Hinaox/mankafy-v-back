...
activityRouter.get('/:id', async (req, res) => {
    try {
        ...
        if (activity) {
            const firstTag = await getFirstTag(activity.id); // Récupère le premier tag
...
activityRouter.get('/:id/full', async (req, res) => {
...
activityRouter.get('/location/:locationId', async (req, res) => {
    ...
    try {
        ...
        if (activities.length == 0) console.log("0 be");
...
activityRouter.post('/:activityId/tags', async (req, res) => {
    const { activityId } = req.params;
...
activityRouter.delete('/:activityId/tags/:tagId', async (req, res) => {
